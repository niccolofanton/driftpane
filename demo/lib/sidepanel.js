...
const pushAnimations = new WeakMap();
let nextPanelId = 0;
...
export class SidepanelController {
    constructor(pane, options = {}) {
        ...
        this.onPointerEnter = (event) => {
            if (this.mode !== 'hover' || event.pointerType === 'touch')
                return;
    ...
    setWidth(width) {
        if (!Number.isFinite(width) || width <= 0 || this.disposed)
            return;
    ...
    setOpen(open, focus = true) {
        ...
        if (open && !wasOpen) {
            ...
            if (this.element.contains(this.pane.ownerDocument.activeElement)) {
                this.pane.ownerDocument.activeElement?.blur();
            }
        }
        this.layout();
    }
    updatePush(width) {
        ...
        if (from.marginLeft === to.marginLeft &&
    ...
    writePush(width) {
        ...
        if (!state)
            return;
        if (width > 0)
            state.panels.set(this, { side: this.side, width });
        else
            state.panels.delete(this);
        if (!state.panels.size) {
            this.target.style.setProperty('margin-left', state.left, state.leftPriority);
            this.target.style.setProperty('margin-right', state.right, state.rightPriority);
        ...
        for (const panel of state.panels.values()) {
            if (panel.side === 'left')
                left = Math.max(left, panel.width);
            else
                right = Math.max(right, panel.width);
        }
        this.target.style.setProperty('margin-left', `calc(${state.baseLeft} + ${left}px)`, state.leftPriority);
        this.target.style.setProperty('margin-right', `calc(${state.baseRight} + ${right}px)`, state.rightPriority);
    }
    dispose() {
        if (this.disposed)
            return;
        ...
        if (this.marker.parentNode)
            this.marker.replaceWith(this.pane);
        else
            this.element.before(this.pane);
        this.element.remove();
    }
}
