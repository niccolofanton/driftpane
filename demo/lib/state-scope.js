...
export function structureSignature(state) {
    const walk = (node) => {
        ...
        if (binding && typeof binding === 'object') {
            sig['b'] = binding['key'];
            sig['l'] = o['label'] ?? null;
            sig['r'] = binding['readonly'] === true;
        }
        if (Array.isArray(o['children'])) {
            sig['c'] = o['children'].map(walk);
        }
        if (Array.isArray(o['pages'])) {
            sig['p'] = o['pages'].map(walk);
        }
        return sig;
    };
...
function isTabPageState(node) {
    return (typeof node['selected'] === 'boolean' &&
 ...
 */
export function importPaneState(pane, state) {
    const before = pane.exportState?.();
    const visit = (api, snapshot) => {
        if (!api ||
            typeof api !== 'object' ||
            !snapshot ||
        ...
        if (Array.isArray(node.pages)) {
            const selected = states.findIndex((page) => !!page &&
                typeof page === 'object' &&
                page['selected'] === true);
    ...
    try {
        if (!pane.importState(state))
            throw new Error('Incomplete state import');
        visit(pane, state);
        return true;
    }
    catch {
