...
function addRuntimeFolder() {
    ...
    $('add-control').disabled = true;
}
try {
    if (localStorage.getItem(dynamicKey) === '1')
        addRuntimeFolder();
}
catch {
...
$('add-control').addEventListener('click', () => {
    ...
    notice('A folder was added after initialization. Change its phase, fold it, then reload.');
});
...
function showPresets() {
    const folder = pane.children.find((child) => 'title' in child && child.title === 'Presets');
    if (folder && 'expanded' in folder)
...
function updateTheme() {
    ...
    for (const button of document.querySelectorAll('[data-theme-choice]')) {
        button.setAttribute('aria-pressed', String(button.dataset.themeChoice === panel.theme.get()));
    }
}
...
updateTheme();
for (const button of document.querySelectorAll('[data-theme-choice]')) {
    button.addEventListener('click', () => panel.theme.set(button.dataset.themeChoice));
}
function shuffle() {
    ...
    PARAMS.foreground = ['#c4c8f1', '#e6bd9c', '#a6d8c8', '#ddacca'][Math.floor(Math.random() * 4)];
    pane.refresh();
}
...
$('reset-position').addEventListener('click', () => panel.draggable.resetPosition());
let dragging = true;
...
$('next-tab').addEventListener('click', () => {
    ...
    const next = (tabs.pages.findIndex((page) => page.selected) + 1) % tabs.pages.length;
...
function openPicker(binding, selector) {
    panel.setMaxHeight(45);
    show(binding.element);
    setTimeout(() => binding.element.querySelector(selector)?.click(), 600);
}
...
$('open-receiver').addEventListener('click', () => {
    ...
    void panel
        ...
        if (payload)
            url.searchParams.set('dp:showcase-receiver', payload);
...
$('export-backup').addEventListener('click', () => download(`driftpane-${namespace}-backup.json`, panel.exportAllJSON()));
const backupInput = $('backup-file');
...
backupInput.addEventListener('change', () => {
    ...
    void file
        ...
        .catch((error) => notice(error instanceof Error
        ? error.message
        : 'Could not import this backup.'));
});
function bindingValue(state, key) {
...
function updateStatus() {
    ...
    if (location.href !== displayedUrl) {
        displayedUrl = location.href;
        shareField.value = new URL(location.href).searchParams.has(`dp:${namespace}`)
            ? location.href
            : '';
    }
}
...
let animation = 0;
const wave = (n) => PARAMS.wave === 'square'
...
function draw(now) {
    ...
    if (!reducedMotion.matches)
        phase += elapsed * 1.2 * PARAMS.speed;
    ...
    for (let layer = 1; layer >= 0; layer--) {
        ...
        for (let x = 0; x <= width; x += 2) {
            const p = (x / Math.max(width, 1)) * Math.PI * 2 * PARAMS.detail +
                phase +
                PARAMS.seed * 0.1;
            const y = height / 2 +
