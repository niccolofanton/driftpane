...
const PARAMS = {
	...
	speed: 0.6,
	amplitude: 48,
	wave: 'sine' as 'sine' | 'square' | 'triangle',
	background: '#0c0c0e',
	foreground: '#cfcfd6',
	thickness: 3,
	glow: true,
 ...
 */
function setTheme(t: 'dark' | 'light', updateSceneColors = true): void {
...
fPrev
	...
	.on('change', (ev) => setTheme(ev.value as 'dark' | 'light'));
...
(window as unknown as {driftpane: typeof panel}).driftpane = panel;
...
if (canvas) {
	...
	const waveValue = (phase: number): number => {
		switch (PARAMS.wave) {
			case 'square':
	...
	const draw = (): void => {
		...
		for (let x = 0; x <= w; x += 2) {
			const phase =
				(x / w) * Math.PI * 2 * PARAMS.detail + t + PARAMS.seed * 0.1;
