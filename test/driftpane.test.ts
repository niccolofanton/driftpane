...
describe('Driftpane facade', () => {
	...
	it('still mounts the preset folder with presetsEnabled:false when urlSync is on (forced)', () => {
		// urlSync (on by default) forces the preset menu on: the share UI lives
		// there. presetsEnabled:false alone is not enough to remove it.
		const drift = createDriftpane(pane, {presetsEnabled: false});
		expect(pane.children).toHaveLength(1);
		drift.dispose();
	});

	it('does NOT mount a preset folder when BOTH presetsEnabled and urlSync are false', () => {
		const drift = createDriftpane(pane, {
